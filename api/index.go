...
// Resumo: Ponto de entrada para Vercel (Serverless). Expõe a função Handler que roteia as requisições HTTP
// para endpoints da API, incluindo health check e stubs de autenticação de admin conforme o README.
...
func adminListHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if v := strings.TrimSpace(q.Get("offset")); v != "" {
        if n, e := strconv.Atoi(v); e == nil && n >= 0 { offset = n }
    }
...
func adminCreateHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if mailer != nil {
        go func() {
            ...
            if cfg.AdminCreatedTemplate != "" { tmpl = cfg.AdminCreatedTemplate }
            verifyURL := buildVerifyURL(r, code)
            data := map[string]any{
...
func authenticateAdmin(r *http.Request) (int64, string, error) {
    ...
    if sub == "" || sro == "" { return 0, "", errors.New("claims incompletas") }
    var id int64 = 0
...
func logDebug(format string, args ...any) { if logEnabled("DEBUG") { log.Printf("[DEBUG] "+format, args...) } }
func logInfo(format string, args ...any)  { if logEnabled("INFO")  { log.Printf("[INFO]  "+format, args...) } }
func logWarn(format string, args ...any)  { if logEnabled("WARN")  { log.Printf("[WARN]  "+format, args...) } }
func logError(format string, args ...any) { if logEnabled("ERROR") { log.Printf("[ERROR] "+format, args...) } }
...
func requestBaseURL(r *http.Request) string {
    ...
    if host == "" { host = strings.TrimSpace(r.Host) }
    if host == "" { return "" }
    return scheme + "://" + host
}
